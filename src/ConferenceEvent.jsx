...
const ConferenceEvent = () => {
        ...
        mealItems.forEach((item) => {
            if (item.selected) {
                ...
                if (item.numberOfPeople) {
                    itemForDisplay.numberOfPeople = item.numberOfPeople;
                }
    ...
    const navigateToProduct = (idType) => {
    ...
    return (
        <>
            <navbar className="navbar_event_conference">
                <div className="company_logo">Conference Event</div>
                <div className="left_navbar">
                    <div className="nav_links">
                        <a href="#venue" onClick={() => navigateToProduct("#venue")}>Venue</a>
                        <a href="#addons" onClick={() => navigateToProduct("#addons")}>Add-Ons</a>
                        <a href="#meals" onClick={() => navigateToProduct("#meals")}>Meals</a>
                                            ...
                                            {venueItems[index].name === "Auditorium Hall (Capacity:200)" 
                                                ? (
                                                    <>
                                                        ...
                                                        <button className={venueItems[index].quantity === 3 ? "btn-success btn-disabled" : "btn-success btn-plus"} onClick={() => handleAddToCart(index)}>
                                                            &#43;
                                                        </button>
                                                    </>
                                                ) : (
                            ...
                            <div id="addons" className="venue_container container_main">
                                ...
                                <div className="addons_selection">
                                    {avItems.map((item, index) => (
                                        <div className="av_data venue_main">
                                            <div className="img">
                                                <img src={item.img} alt={item.name} />
                                            </div>
                                            <div className="text">{item.name}</div>
                                            ...
                                            <div className="addons_btn">
                                                <button className="btn-warning" onClick={() => dispatch(decrementAvQuantity(index))}>
                                                    &ndash;
                                                </button>
                                                <span className="quantity-value">{item.quantity}</span>
                                                <button className="btn-success" onClick={() => dispatch(incrementAvQuantity(index))}>
                                                    &#43;
                                                </button>
                                            </div>
                                        </div>
